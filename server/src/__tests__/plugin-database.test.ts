...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  it("applies multi-statement migrations that alter a table and create an index", async () => {
    ...
    const packageRoot = await createPluginPackage(
      pluginManifest,
      `
      CREATE TABLE ${namespace}.org_units (
        id uuid PRIMARY KEY,
        company_id uuid NOT NULL
      );

      ALTER TABLE ${namespace}.org_units
        ADD COLUMN IF NOT EXISTS leader_agent_id text;

      CREATE INDEX IF NOT EXISTS ${namespace}_org_units_leader_agent_idx
        ON ${namespace}.org_units (company_id, leader_agent_id)
        WHERE leader_agent_id IS NOT NULL;
      `,
    );
    const pluginId = await installPluginRecord(pluginManifest);
